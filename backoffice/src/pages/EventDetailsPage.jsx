...
import { Button, Field, Notice, StatusPill } from '../components/ui';
    ...
    <section className="px-6 pb-8 pt-6 max-[760px]:p-4">
      ...
      <div className="rounded border border-neutral-200 bg-white p-5">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2.5 text-neutral-500">
          ...
          <span className="inline-flex min-h-[26px] items-center rounded-full border border-neutral-200 bg-neutral-50 px-2.5 text-xs font-extrabold text-neutral-950">{formatDate(event.startsAt)}</span>
          {event.location ? (
            <span className="inline-flex min-h-[26px] max-w-[min(360px,100%)] items-center gap-1.5 overflow-hidden text-ellipsis whitespace-nowrap rounded-full border border-neutral-200 bg-neutral-50 px-2.5 text-xs font-extrabold text-neutral-950">
              <MapPin size={14} />
              {event.location}
            </span>
          ) : null}
      ...
      <div className="mt-3.5 grid grid-cols-2 gap-3.5 max-[760px]:grid-cols-1">
        <div className="rounded border border-neutral-200 bg-white p-[18px]">
      ...
      <div className="mt-3.5 grid gap-3">
        {isLoading ? (
          <div className="inline-flex items-center gap-2 rounded border border-neutral-200 bg-white p-[18px]">
            <Loader2 className="animate-spin" size={16} />
            Chargement des badges...
          </div>
        ) : null}
        {!isLoading && accessRoles.length === 0 ? (
          <div className="rounded border border-neutral-200 bg-white p-[18px] font-extrabold text-neutral-500">
            <p>Aucun badge QR pour cet evenement.</p>
          </div>
        ) : null}
        {accessRoles.map((role) => (
          <article className="grid grid-cols-[minmax(0,1fr)_180px] items-start gap-[22px] rounded border border-neutral-200 bg-white p-[18px] max-[760px]:grid-cols-1" key={role.id}>
            ...
            <div className="grid w-[180px] gap-2.5 max-[760px]:w-[min(220px,100%)]">
              {role.qrCodeDataUrl ? (
                <img className="aspect-square h-auto w-full rounded border border-neutral-200" src={role.qrCodeDataUrl} alt={`QR ${role.name}`} />
              ) : (
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                {role.qrCodeDataUrl ? (
                  <a className="grid min-h-9 place-items-center rounded border border-neutral-200 bg-white text-neutral-950" href={role.qrCodeDataUrl} download={`qr-${event.slug}-${role.name}.png`} title="Telecharger le QR">
                    <Download size={16} />
                  </a>
                ) : null}
